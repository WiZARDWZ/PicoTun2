...
type StealthConfig struct {
	...
	// v2.5.1: Anti-DPI rotation — each connection uses different fingerprint
...
func applyBaseDefaults(c *Config) {
	...
	if c.Smux.MaxRecv <= 0 {
		c.Smux.MaxRecv = 2097152 // v2.5.1: 2MB for speed
	}
	if c.Smux.MaxStream <= 0 {
		c.Smux.MaxStream = 2097152 // v2.5.1: 2MB for speed
	}
	if c.Smux.FrameSize <= 0 {
		c.Smux.FrameSize = 8192 // v2.5.1: 8KB for speed
	}
	if c.Smux.Version <= 0 {
		c.Smux.Version = 2
	}
	...
	if c.Advanced.TCPReadBuffer <= 0 {
		c.Advanced.TCPReadBuffer = 131072 // v2.5.1: 128KB for speed
	}
	if c.Advanced.TCPWriteBuffer <= 0 {
		c.Advanced.TCPWriteBuffer = 131072 // v2.5.1: 128KB for speed
	}
	...
	if c.Stealth.MaxBurstSize <= 0 {
		c.Stealth.MaxBurstSize = 8192
	}
	if c.Stealth.FakeTrafficInterval <= 0 {
		c.Stealth.FakeTrafficInterval = 30
	}

	// v2.5.1: Domain/UA rotation pools for DPI evasion
	if len(c.Stealth.DomainPool) == 0 {
		c.Stealth.DomainPool = []string{
			"www.google.com", "www.bing.com", "www.microsoft.com",
			"www.apple.com", "support.google.com", "accounts.google.com",
			"cloud.google.com", "docs.google.com", "mail.google.com",
			"www.cloudflare.com", "cdn.jsdelivr.net", "ajax.googleapis.com",
			"fonts.googleapis.com", "maps.googleapis.com",
			"www.gstatic.com", "ssl.gstatic.com",
		}
	}
	if len(c.Stealth.UAPool) == 0 {
		c.Stealth.UAPool = []string{
	...
	// v2.5.1: Auto-enable rotation for httpmux/httpsmux transports
	if transport == "httpmux" || transport == "httpsmux" {
		if !c.Stealth.RotateDomain {
			c.Stealth.RotateDomain = true
		}
		if !c.Stealth.RotateUA {
			c.Stealth.RotateUA = true
		}
		if !c.Stealth.RandomPadding {
			c.Stealth.RandomPadding = true
		}
		if !c.Stealth.BurstSplit {
			c.Stealth.BurstSplit = true
		}
		if !c.Stealth.FakeTraffic {
			c.Stealth.FakeTraffic = true
		}
	}
...
func migrateConfig(c *Config, path string) {
	...
	if c.ConfigVersion < 2 {
		if c.Smux.MaxRecv == 524288 {
			c.Smux.MaxRecv = 2097152
			log.Printf("[CONFIG]   smux.max_recv: 512KB → 2MB")
		}
		if c.Smux.MaxStream == 524288 {
			c.Smux.MaxStream = 2097152
			log.Printf("[CONFIG]   smux.max_stream: 512KB → 2MB")
		}
		if c.Smux.FrameSize == 2048 {
			c.Smux.FrameSize = 8192
			log.Printf("[CONFIG]   smux.frame_size: 2KB → 8KB")
		}
		if c.Smux.KeepAlive == 1 && c.Profile != "gaming" {
			c.Smux.KeepAlive = 2
			log.Printf("[CONFIG]   smux.keepalive: 1s → 2s")
		}
		if c.Advanced.TCPReadBuffer == 32768 {
			c.Advanced.TCPReadBuffer = 131072
			log.Printf("[CONFIG]   tcp_read_buffer: 32KB → 128KB")
		}
		if c.Advanced.TCPWriteBuffer == 32768 {
			c.Advanced.TCPWriteBuffer = 131072
			log.Printf("[CONFIG]   tcp_write_buffer: 32KB → 128KB")
		}
	...
	// v2.5.1: Upgrade speed + DPI defaults
	if c.ConfigVersion < 3 {
		if c.Smux.MaxRecv == 1048576 {
			c.Smux.MaxRecv = 2097152
			log.Printf("[CONFIG]   smux.max_recv: 1MB → 2MB")
		}
		if c.Smux.MaxStream == 1048576 {
			c.Smux.MaxStream = 2097152
			log.Printf("[CONFIG]   smux.max_stream: 1MB → 2MB")
		}
		if c.Smux.FrameSize == 4096 {
			c.Smux.FrameSize = 8192
			log.Printf("[CONFIG]   smux.frame_size: 4KB → 8KB")
		}
		if c.Advanced.TCPReadBuffer == 65536 {
			c.Advanced.TCPReadBuffer = 131072
			log.Printf("[CONFIG]   tcp_read_buffer: 64KB → 128KB")
		}
		if c.Advanced.TCPWriteBuffer == 65536 {
			c.Advanced.TCPWriteBuffer = 131072
			log.Printf("[CONFIG]   tcp_write_buffer: 64KB → 128KB")
		}
		if c.Stealth.MaxBurstSize == 4096 {
			c.Stealth.MaxBurstSize = 8192
			log.Printf("[CONFIG]   stealth.max_burst: 4KB → 8KB")
		}
	}
